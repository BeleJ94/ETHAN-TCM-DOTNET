...
public interface IAccessAdministrationService
{
    ...
    Task<AccessOperationResult> UpdateUserAsync(Guid id, bool isActive, IReadOnlyCollection<Guid> roleIds, string reason, CancellationToken cancellationToken = default);
...
public sealed record AccessUserDetails(Guid Id, string Login, string DisplayName, string Email, string? ExternalId, bool IsActive, DateTimeOffset? LastSyncedAt, IReadOnlyCollection<AccessRoleOption> Roles, IReadOnlyCollection<string> EffectivePermissions);
